...
int main()
{
    ...
    for (int i = 1; i <= numberOfProduct; i++)
    {
        for (int j = 1; j <= capacity; j++)
        {
            ...
            {
                Table[i][j] = max(Table[i - 1][j], Table[i][j - prod[i - 1].weight] + prod[i - 1].price);
            }
            else
            {
    ...
    cout << Table[numberOfProduct][capacity] << endl;
