...
bool Board::is_solvable() {
    if (this->dimension & 1) {
        ...
        if (((this->get_blank_row_number() % 2 == 0) && (this->get_number_of_inversions() & 1)) || 
            ((this->get_blank_row_number() & 1) && (this->get_number_of_inversions() % 2 == 0))) {
            return true;
        }
        return false;
    }
...
void solve_npuzzle_manhattan(Board *_initial_board) {
    ...
    while (!manhattan_queue.empty()) {
        ...
        if (visited.count(current_board->get_grid()))
            continue;

        visited.insert(current_board->get_grid());
    ...
    cout << "Minimum number of moves: " << number_of_moves << "\n";
    cout << "Number of explored boards: " << number_of_explored_boards << "\n";
    cout << "Number of expanded boards: " << number_of_expanded_boards << "\n";
    while (!moves.empty()) {
        cout << *moves.top();
        moves.pop();
    }
